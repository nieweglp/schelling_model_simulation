...
def check_local_area(local_people):
    ...
    local_area_ratio = {}
    local_area_ratio['population_first_ratio'] = local_people.count(
        1) / len(local_people)
    local_area_ratio['population_second_ratio'] = local_people.count(
        -1) / len(local_people)
    local_area_ratio['neutral_ratio'] = local_people.count(
        0) / len(local_people)
...
def move_on(world_map, threshold):
